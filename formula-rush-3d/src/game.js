import * as THREE from 'https://cdn.jsdelivr.net/npm/three@0.166.1/build/three.module.js';
import { TEAMS, TRACKS, createDrivers } from './config.js';
import { AudioEngine } from './audio.js';
import { environmentMethods } from './environment.js';
import { sessionsMethods } from './sessions.js';
import { drivingMethods } from './driving.js';

export class FormulaRushGame {
  constructor(canvas, callbacks = {}) {
      this.canvas = canvas;
      this.cb = callbacks;
      this.renderer = new THREE.WebGLRenderer({ canvas, antialias: true, powerPreference: 'high-performance' });
      this.renderer.setPixelRatio(Math.min(1.5, window.devicePixelRatio || 1));
      this.renderer.shadowMap.enabled = true;
      this.renderer.shadowMap.type = THREE.PCFSoftShadowMap;
      this.renderer.outputColorSpace = THREE.SRGBColorSpace;
      this.renderer.toneMapping = THREE.ACESFilmicToneMapping;
      this.renderer.toneMappingExposure = 1.1;
      this.scene = new THREE.Scene();
      this.camera = new THREE.PerspectiveCamera(58, 1, 0.1, 900);
      this.audio = new AudioEngine();
      this.inputs = { left: false, right: false, throttle: false, brake: false, ers: false };
      this.drivers = createDrivers();
      this.team = TEAMS[1];
      this.trackIndex = 0;
      this.session = 'Q1';
      this.running = false;
      this.paused = true;
      this.countdown = 0;
      this.lastTime = 0;
      this.trackGroup = new THREE.Group();
      this.dynamicGroup = new THREE.Group();
      this.scene.add(this.trackGroup, this.dynamicGroup);
      this.qualifying = {};
      this.finalGrid = [];
      this.championship = this.drivers.map((driver) => ({ driver, points: 0 }));
      this.message = 'TIENI PREMUTO ACCELERA';
      this.messageTimer = 0;
      this.setupLighting();
      this.resize();
      this.loadTrack(0);
      window.__FORMULA_RUSH__ = {
        getState: () => this.debugState(),
        setInput: (key, value) => this.setInput(key, value),
        completeSession: () => this.debugCompleteSession(),
        skipCountdown: () => { this.countdown = -1; },
        step: (seconds, dt = 1 / 60) => { for (let t = 0; t < seconds; t += dt) this.update(dt); },
        game: this,
      };
      requestAnimationFrame((time) => this.loop(time));
    }

  selectTeam(teamId) {
      this.team = TEAMS.find((team) => team.id === teamId) || TEAMS[0];
    }

  setAudioEnabled(enabled) {
      return this.audio.setEnabled(enabled);
    }

  resetChampionship() {
      this.championship.forEach((entry) => { entry.points = 0; });
      this.trackIndex = 0;
      this.qualifying = {};
      this.finalGrid = [];
      this.loadTrack(0);
    }

  beginChampionship() {
      this.resetChampionship();
      this.prepareSession('Q1');
    }

  getMiniMapData(count = 100) {
      return Array.from({ length: count }, (_, index) => this.curve.getPointAt(index / count));
    }

  debugState() {
      return {
        session: this.session,
        speed: this.player?.speed || 0,
        lateral: this.player?.lateral || 0,
        throttle: this.inputs.throttle,
        audioEnabled: this.audio.enabled,
        trackChildren: this.trackGroup.children.length,
        treeCount: 0,
        running: this.running,
        countdown: this.countdown,
        paused: this.paused,
        totalCars: this.player?.totalCars,
        position: this.player?.position,
      };
    }

  debugCompleteSession() {
      if (!this.running) return false;
      if (this.session === 'RACE') this.player.distance = this.track.laps * this.trackLength + 1;
      else this.player.distance = this.trackLength + 1;
      return true;
    }

  loop(time) {
      const dt = Math.min(0.033, this.lastTime ? (time - this.lastTime) / 1000 : 0.016);
      this.lastTime = time;
      this.update(dt);
      this.renderer.render(this.scene, this.camera);
      requestAnimationFrame((next) => this.loop(next));
    }
}

Object.assign(FormulaRushGame.prototype, environmentMethods, sessionsMethods, drivingMethods);
